...
Source: https://wiki.aoe.top/ModTutorial/UE4SS/guides/creating-a-lua-mod.html
Friendly site: 小莫Wiki
Group: AOE.TOP
Fetched: 2026-06-18T02:26:56.904Z
Status: 200
...
- 了解 如何安装UE4SS 到目标游戏中并确保其正常运行；
- 能够编写基本的Lua代码（可参考官方书籍 《Programming in Lua》 及其后续版本，或网上其他推荐的教程）；
- 了解虚幻引擎的对象模型和游戏修改的基础知识。

## 最小Lua模组是什么样的 ​

UE4SS中的Lua模组是一组放置在UE4SS安装目录的 Mods/ 文件夹内的Lua脚本。 以本例为目的，让我们称其为 MyLuaMod 。

为了使模组能被加载和执行：

- 模组文件夹必须有一个 scripts 子文件夹和其中的 main.lua 文件，结构如下：
...
- Mods\MyLuaMod\scripts\main.lua 文件中必须包含一些Lua代码，例如：
lua
print ( " [MyLuaMod] 模组已加载 \n " )

1

- 必须在 Mods\mods.txt 中添加并启用模组，添加一行包含模组文件夹名称（即模组名称）和 1 表示启用或 0 表示禁用：
...
UE4SS提供的API在此文档的 "Lua API" 章节的子部分中有详细说明。使用这些函数和类，您可以在内存中查找和操作虚幻引擎对象的实例，创建新对象或修改现有对象，调用它们的方法并访问它们的字段。
...
如果您需要加载具有原生代码的Lua C库（即在Windows上使用DLL）， 您可以直接将这些DLL放在同一个 \scripts\ 文件夹中。

# 设置Lua模组开发环境 ​

如果您的代码编辑器或IDE为Lua开发正确配置并了解UE4SS API，编写模组会容易得多。

- 配置您的代码编辑器/IDE以支持Lua语法高亮和代码自动完成。如果您使用VSCode，请参阅 使用自定义Lua绑定 。
- 确保您的UE4SS构建包含 Mods\shared\Types.lua （从Github发布的开发版本包含它）。这将在您的IDE中加载UE4SS API定义。
- （可选）从UE4SS Gui控制台 转储Lua绑定 ，并 按照建议加载它们 。
...
- 使用 UE4SS的 UEHelpers 类 获取 玩家控制器 。
- 获取 Pawn ，它代表玩家在虚幻引擎中可以控制的实际"物理"实体。
- 调用适当的虚幻引擎 方法 K2_GetActorLocation ，它返回 Pawn 的位置（通过访问其父 Actor 类）。
- 位置是虚幻引擎类型 FVector 的三分量向量，具有 X 、 Y 和 Z 作为其字段。
lua
