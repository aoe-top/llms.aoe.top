# 使用 CMake 工具包进行构建 | 小莫的维基库

Source: https://wiki.aoe.top/CMake/build.html
Friendly site: 小莫Wiki
Group: AOE.TOP
Fetched: 2026-06-18T02:27:00.501Z
Status: 200
...
一旦您已经 配置 好了项目，就可以运行 CMake 构建。您在 CMake 工具包中大部分时间都会花费在配置构建上。构建过程要简单得多。

您可以通过以下方式启动构建：

- 从 VS Code 命令板运行 CMake: Build 。
- 按下键盘快捷键（默认为 F7 ）。
- 在 VS Code 状态栏中选择 Build 按钮。
...
构建的结果将写入由 cmake.buildDirectory 指定的目录中。这默认为项目目录的子目录，因此构建结果在项目资源管理器中可见。确切的文件路径将取决于您的 CMake 项目配置。
...
CMake 工具包通过将 --build 标志传递给 CMake 进行构建。该标志用作与生成器无关的构建调用工具。CMake 工具包还会传递 --config <build-type> ，基于活动 变体 中的当前构建类型。这指示多配置构建工具应构建哪个配置。

对于高级场景，可以使用 cmake.buildArgs 设置附加 --build 标志，并且可以使用 cmake.buildToolArgs 设置底层构建工具的额外标志。

### 构建环境 ​

环境变量从调用 Visual Studio Code 进程继承，同时也包含从 cmake.buildEnvironment 和 cmake.environment 设置获取的其他变量。

如果您正在使用 Visual Studio Kit ，则 CMake 工具包会使用适当的环境变量集以使用所选版本的 Visual Studio 进行构建，就像它在 CMakeTools 配置步骤 期间设置这些环境变量一样。
...
- 使用 CMake 工具包进行 启动和调试 。
- 了解 可以设置的 CMake 设置 以控制构建。
- 探索 CMake 工具包文档 。
