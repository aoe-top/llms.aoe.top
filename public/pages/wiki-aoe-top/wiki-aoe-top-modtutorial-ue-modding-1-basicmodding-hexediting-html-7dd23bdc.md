...
Source: https://wiki.aoe.top/ModTutorial/UE_Modding/1.BasicModding/HexEditing.html
Friendly site: 小莫Wiki
Group: AOE.TOP
Fetched: 2026-06-18T02:29:03.428Z
Status: 200
...
导出的文件成对出现，包含 .uasset 和 .uexp 。
值存储在 .uexp 文件中，所以这是我们需要编辑的文件。

用十六进制编辑器打开 .uexp 文件（我使用的是Hex Editor Neo）。
按Ctrl+H打开替换窗口（根据你使用的软件，快捷键可能不同）。
确保类型是 Float ，然后输入原始值和目标值。
注意： 如果你的编辑器没有这个替换功能，请尝试 方法2 。
