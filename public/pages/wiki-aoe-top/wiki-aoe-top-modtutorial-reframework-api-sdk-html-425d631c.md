...
Source: https://wiki.aoe.top/ModTutorial/REFramework/api/sdk.html
Friendly site: 小莫Wiki
Group: AOE.TOP
Fetched: 2026-06-18T02:27:03.670Z
Status: 200
...
使用给定的 type 和 length 创建并返回一个新的 SystemArray 。

type 可以是以下任何一种：

- 从 sdk.typeof 返回的 System.Type
- 从 sdk.find_type_definition 返回的 RETypeDefinition
- 表示类型名称的 Lua string 。
任何其他类型都将抛出 Lua 错误。
...
如果给定 value ，则返回一个类型为 System.SByte 的完全构建好的 REManagedObject 。

### sdk.create_byte(value) ​

如果给定 value ，则返回一个类型为 System.Byte 的完全构建好的 REManagedObject 。

### sdk.create_int16(value) ​

如果给定 value ，则返回一个类型为 System.Int16 的完全构建好的 REManagedObject 。

### sdk.create_uint16(value) ​

如果给定 value ，则返回一个类型为 System.UInt16 的完全构建好的 REManagedObject 。

### sdk.create_int32(value) ​

如果给定 value ，则返回一个类型为 System.Int32 的完全构建好的 REManagedObject 。

### sdk.create_uint32(value) ​

如果给定 value ，则返回一个类型为 System.UInt32 的完全构建好的 REManagedObject 。

### sdk.create_int64(value) ​

如果给定 value ，则返回一个类型为 System.Int64 的完全构建好的 REManagedObject 。

### sdk.create_uint64(value) ​

如果给定 value ，则返回一个类型为 System.UInt64 的完全构建好的 REManagedObject 。

### sdk.create_single(value) ​

如果给定 value ，则返回一个类型为 System.Single 的完全构建好的 REManagedObject 。

### sdk.create_double(value) ​

如果给定 value ，则返回一个类型为 System.Double 的完全构建好的 REManagedObject 。
...
返回一个 REManagedObject ，它是 via.UserData 。 typename 可以为 "via.UserData" ，除非你知道完整的 typename。

### sdk.deserialize(data) ​

从 data 生成 REManagedObject 列表。
...
只应与本机类型一起使用，而不与 REManagedObject 一起使用（尽管如果需要，也可以这样做）。
...
返回一个 REManagedObject* 。返回引擎正在使用的当前相机。

### sdk.hook(method_definition, pre_function, post_function, ignore_jmp) ​

为 method_definition 创建挂钩，拦截游戏对其进行的所有调用。

ignore_jmp - 跳过尝试跟踪函数中的第一个 jmp。默认值为 false 。

使用 pre_function 和 post_function ，可以修改这些函数的行为。

注意：某些本机方法可能无法使用此方法挂钩，例如它们只是本机函数的包装器。我们需要从我们的端口完成一些额外的工作才能使它们正常工作。 预处理函数和后处理函数如下所示：
...
如果 value 是有效的 REManagedObject ，则返回 true。

仅在必要时使用。执行一堆检查并频繁调用 IsBadReadPtr 。

### sdk.to_managed_object(value) ​

尝试将 value 转换为 REManagedObject* 。

value 可以是以下任意类型：

- REManagedObject* ，在这种情况下原样返回
- 可转换为 uintptr_t 的 lua 数字，表示对象的地址
- void*
任何其他类型都将返回 nil 。

不是有效的 REManagedObject* 的 value 将返回 nil ，相当于在其上调用 sdk.is_managed_object 。
